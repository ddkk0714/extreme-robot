...
대회 5개 구간(봄_정찰(IFF)/여름_구호물자운반/가을_장애물식별/겨울_경로확보/정찰동행_추종)이
...
MODEL_PRESETS = {
    "box": {
        # 구간2 여름_구호물자운반 — ARM_GRASP_BOX. seg 모델(1클래스 box-segmentation)
    ...
    "iff": {
        # 구간1 봄_정찰(IFF, 군복 식별) — 파워트레인 소유 로직이 /detected_objects로 소비.
        # detect 전용 모델(마스크 없음) → translation은 bbox 중심 depth 폴백,
        # orientation은 채워지지 않음. 관찰 전용(집지 않음) → pick_classes 비워서
        # /pick_target 후보에서 자동 제외.
    ...
    "vision_marker": {
        # 구간3 가을_장애물식별 — 비전마커 식별, 자세 락만(집지 않음). 파워트레인 소유
    ...
    "winter_terrain": {
        # 구간4 겨울_경로확보(빙판/제설) — 자세 락만(집지 않음). ⚠️ TODO: 이 구간이
    ...
    "lead_robot": {
        # 구간5 정찰동행_추종 — 선도봇 PID 추종, 자세 락만(집지 않음). 파워트레인 소유
