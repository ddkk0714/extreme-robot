...
def stage_grasp(args):
    ...
    if not 1 <= args.profile_acceleration <= 20:
        raise CalibrationError("profile acceleration must be in [1, 20]")
    if not 1 <= args.profile_velocity <= 50:
        raise CalibrationError("profile velocity must be in [1, 50]")
